...
class PriceTransformationService {
    ...
    public function num2bgtext($number, $stotinki = false) {
        ...
        if ($number > 19 && $number < 100) {
            ...
            $tmp = $_mod10 ? $tmp . " и " . $this->num2bgtext($_mod10, $stotinki) : $tmp;
            return $tmp;
        }
        /* До хиляда */
        ...
        if ($number > 999 && $number < 1000000) {
            ...
            $tmp = ($_div1000 == 1) ? "хиляда" :
                    (($_div1000 == 2) ? "две хиляди" : $this->num2bgtext($_div1000) . " хиляди");
            $_num0[2] = "два";
            ...
            if ($_mod1000) {
                $tmp .= " " . $this->num2bgtext($_mod1000);
            }
            return $tmp;
        }
        /* Над милион */
        if ($number > 999999 && $number < 1000000000) {
            $tmp = ($_div1000000 == 1) ? "един милион" : $this->num2bgtext($_div1000000) . " милиона";
            ...
            if ($_mod1000000) {
                $tmp .= " " . $this->num2bgtext($_mod1000000);
            }
            return $tmp;
        }
        /* Над милиард */
        if ($number > 99999999 && $number <= 2000000000) {
            ...
            if ($_mod1000000000) {
                $tmp .= " " . $this->num2bgtext($_mod1000000000);
            }
            return $tmp;
        }
        return "";
    ...
    public function number2currency($number, $currency) {
        list($lv, $st) = explode(".", number_format($number, 2, ".", ""));
        $lv = (int) $lv;
        if ($lv >= 2000000000) {
            return "Твърде голямо число";
        }
        $text = $this->num2bgtext($lv);

        if(($currency instanceof \BackedEnum && $currency->name == 'BGN') || $currency == 'BGN') {
            $text .= $lv == 1 ? " лев" : " лева";
        } elseif(($currency instanceof \BackedEnum && $currency->name == 'EURO') || $currency == 'EURO') {
            $text .= " евро";
        }
        if ($st <> 0) {
        ...
        if ($st && $st != 0) {
            $sttext = $this->num2bgtext($st, true);
            $text .= " и " . $this->num2bgtext($st, true);
            if(($currency instanceof \BackedEnum && $currency->name == 'BGN') || $currency == 'BGN') {
                $text .= $st == 1 ? " стотинка" : " стотинки";
            } elseif(($currency instanceof \BackedEnum && $currency->name == 'EURO') || $currency == 'EURO') {
                $text .= $lv == 1 ? " цент" : " цента";
            }
        }
        return $text;
    }

    public function number2currencyNew($number, $currency) {
        list($lv, $st) = explode(".", number_format($number, 2, ".", ""));
        $lv = (int) $lv;
        if ($lv >= 2000000000) {
            return "Твърде голямо число";
        ...
        $text = $numberTransformer->toWords($lv);
        ...
        if ($st && $st != 0) {
            $sttext = $numberTransformer->toWords($st);
            $text .= " and " . $numberTransformer->toWords($st);
            if(($currency instanceof \BackedEnum && $currency->name == 'BGN') || $currency == 'BGN') {
                $text .= $st == 1 ? " stotinka" : " stotinki";
            } elseif(($currency instanceof \BackedEnum && $currency->name == 'EURO') || $currency == 'EURO') {
                $text .= $lv == 1 ? " cent" : " cent";
            }
        }
